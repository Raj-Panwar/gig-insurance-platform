...
export default function RegisterScreen({ navigation }) {
  const [form, setForm] = useState({ name: '', phone: '', city: '', platform: '' });
  ...
  const handleRegister = async () => {
    ...
    setLoading(true);
    try {
      await registerWorker(form.name.trim(), form.phone.trim(), form.city.trim(), form.platform.trim());
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={[styles.card, SHADOW.lg]}>
          ...
          <Text style={styles.cardSubtitle}>
            Register as a gig worker to get income protection
          </Text>
          ...
          <Input label="Your City"     value={form.city}     onChangeText={v => set('city', v)}
                 placeholder="e.g. Delhi" error={errors.city} />
...
const styles = StyleSheet.create({
  ...
  card:                { backgroundColor: COLORS.white, borderRadius: 28,
  ...
  errorBanner:         { backgroundColor: COLORS.dangerLight, borderRadius: RADIUS.md,
