...
export default function ClaimsScreen() {
  ...
  const handleManualClaim = async () => {
    ...
    try {
      ...
      // Send as a trigger event with the user's current location
      await apiClient.post('/trigger/event', {
        event_type: manualType,
        location:   city,
        severity:   60, // above threshold to ensure claim is created
      });

      setShowManual(false);
      Alert.alert('✅ Claim Submitted', 'Your manual claim has been submitted for review.');
      load();
    } catch (err) {
