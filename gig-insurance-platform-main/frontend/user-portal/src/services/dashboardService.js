...
export const getWorkerDashboard = async (user_id) => {
  const res = await apiClient.get(`/dashboard/${user_id}`);
  return res.data;
};

// ── Worker Claims ─────────────────────────────────────────────────────────────
// GET /worker/claims/<user_id>
// Returns: { user_id, total_claims, claims: [{ claim_id, trigger_type, status, payout, created_at }] }
...
// Returns: { user_id, payouts, total_amount, payouts: [{ transaction_id, amount, status, processed_at }] }
