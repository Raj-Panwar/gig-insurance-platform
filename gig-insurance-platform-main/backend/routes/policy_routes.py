...
@cross_origin()
def create_policy():
    print("🔥 CREATE POLICY API CALLED")
    ...
    # 🔥 STEP 1: deactivate old active policy
    Policy.query.filter_by(user_id=data["user_id"], status="ACTIVE").update({
        "status": "INACTIVE"
    })
    policy = Policy(
    ...
    db.session.commit()
    print("🔥 POLICY SAVED:", policy.user_id, policy.status)
